...
world_day_dict = {}
f = open("montreal_user_domains.csv")
for line in f:
    line = line.strip().split(";")
    day = line[0]
    d = json.loads(line[1])
...
w = open("diff_info_rankings_april.csv","w+")
w.write("USER_SCORE,TORONTO,STRESS,TORONTO_STRESS\n")
day_aves = {}
day_counts = {}
    ...
    for user in day_dict[day]:
        ...
        if user_count > 0:
            ...
            if -30 <= day_sign <= 30:
                ...
                day_counts[day] += 1
                w.write("{},{},{},{}\n".format(user_score,1,day_flag,day_flag))
...
for day in sorted(list(world_day_dict.keys())):
    for user in world_day_dict[day]:
        ...
        if user_count > 0:
            ...
            if day_sign > 0: day_flag = 0
            if -30 <= day_sign <= 30:
                w.write("{},{},{},{}\n".format(user_score,0,day_flag,0))
                if day not in day_aves: 
                    day_aves[day] = 0
                    day_counts[day] = 0
...
day_aves = {x:day_aves[x]/day_counts[x] for x in day_aves}
for x in day_aves:
    print("{},{}".format(x,day_aves[x]))
